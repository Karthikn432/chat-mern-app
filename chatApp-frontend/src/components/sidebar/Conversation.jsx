...
import { useGetLastMessageTimeQuery, useUnreadMssagesQuery } from '../../features/rtkquery/app-query/usersQuery'
import { extractTime } from '../../utils/utils'

const Conversation = ({ name, profile, id, isLastIdx, selectedConversation, setSelectedConversation, unviewedCount}) => {
    ...
    // const {data:unreadMessages} = useUnreadMssagesQuery(id)
        ...
        <>
            <div className={`flex gap-2 items-center hover:bg-yellow-500 rounded p-2 py-1 cursor-pointer ${getSelectedUser?.id === id ? "bg-sky-500" : ""}`} onClick={() => handleSelectChatUser(id, name, profile)}>
                ...
                <div className="flex flex-col flex-1">
                    ...
                    {unviewedCount > 0 && (
                        <span className="text-sm text-red-500">Unread messages: {unviewedCount}</span>
                    )}
                </div>
            </div>
            {
