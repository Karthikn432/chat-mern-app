...
const Conversation = ({ name, profile, id, isLastIdx, selectedConversation, setSelectedConversation }) => {
    ...
    const handleSelectChatUser = async (id, name, profile) => {
        ...
        localStorage.setItem('selected_conversation', selectedConversation)
    }

    return (
        <>
            <div className={`flex gap-2 items-center hover:bg-yellow-500 rounded p-2 py-1 cursor-pointer ${selectedConversation === id ? "bg-sky-500" : ""}`} onClick={() => handleSelectChatUser(id, name, profile)}>
